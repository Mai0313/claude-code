...
1) 檢查 Node.js（需要 v22+）。在 macOS/Linux 嘗試自動安裝；Windows 會使用隨包的 Node.js 壓縮檔安裝到 `C:\\Program Files\\nodejs4claude`，將其加入使用者 PATH 並設定 NODE_HOME，並同步更新當前行程環境以便立即可用。
...
- 若未安裝或版本低於 v22 的 Node.js：
   ...
   - Windows：安裝程式會使用隨附的 `node-v22.18.0-win-x64.zip`，解壓至 `C:\\Program Files\\nodejs4claude`，並永久更新使用者 PATH/NODE_HOME。若因權限無法寫入 `Program Files`，請以系統管理員身分執行再試。
...
- Windows 安裝 Node.js
   - 安裝程式會使用隨附的 Node.js 壓縮檔並安裝到 `C:\\Program Files\\nodejs4claude`。若解壓失敗（權限問題），請改以系統管理員權限執行，或手動安裝 Node.js v22+ 後再試。

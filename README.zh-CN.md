...
3. **创建配置文件**
   - 在 `~/.claude/settings.json` 创建默认配置
   - 如文件已存在，会询问是否覆盖并自动备份旧配置
...
1. **开始安装**：按下 Enter 键或输入 `1` 并按 Enter
...
- 🐛 **问题报告**：[Claude Code Issues](https://gitea.mediatek.inc/IT-GAIA/claude-code/issues)
...
| 文件/目录 | 用途 |
|-----------|------|
| `~/.claude/settings.json` | 主要配置文件，包含 API 设置和用户偏好 |
| `~/.claude/nodejs/` | Windows 专用：内置的 Node.js 安装目录 |
| `~/.claude/settings.backup_*.json` | 自动备份的旧配置文件 |
